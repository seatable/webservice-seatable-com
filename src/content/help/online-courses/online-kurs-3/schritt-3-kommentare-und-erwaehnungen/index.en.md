...
Every row has its own comment thread. In your window 🌐, open the `Customers` table and pick the customer `James Bennett` to talk about. There are two ways to open his comments:
...
A comment column opens on the right. Type a message — for example, asking a colleague to confirm `James Bennett`'s `Industry` — and click `Submit`. A small speech-bubble icon now appears in the first column of that row, so anyone scanning the table can see a conversation is in progress.
...
Note that a comment, on its own, reaches no one. Switch to Thomas's window 🕶 and look at his notification bell. Nothing. The comment you just wrote is sitting on the row, perfectly visible if Thomas happens to open it, but SeaTable did not tell him it exists.
...
Back in your window 🌐, write a second comment on the same row, and this time mention Thomas directly: type {{< key "@" >}} followed by his name, `Thomas`, and pick him from the list that appears. (You can also use the plus icon above the comment field to add him.) Phrase it as a real request — for instance, asking him to update `James Bennett`'s `Industry` and let you know when it is done. Submit the comment.

{{< warning headline="You can only mention collaborators" text="A name only appears in the mention list if that person already has access to the base. This is why Step 2 came first: because you shared the base with Thomas, you can now mention him. Someone who is merely a member of your team, but has not been given access to this base, cannot be mentioned here." />}}

## The other side of the conversation

Now switch to Thomas's window 🕶. This time his notification bell shows a new alert — and because this one comes from inside the base, it is waiting for him there, not on the home page where his share notification appeared in Step 2. Open it, and you land straight on your comment and the row it belongs to. The mention is what made the difference: an @mention sends a notification, a plain comment does not.

Now act on the request. Because Thomas has read-write access, you can update `James Bennett`'s `Industry` right there, then reply in the same thread to say it is done. The whole discussion stays attached to the record — the question, the change, and the confirmation, all in one place.

{{< warning headline="Some data describes the company, not the contact" text="Industry describes the company, Indelo, rather than James Bennett himself — yet here it sits on each customer row. Indelo has another contact in this base, Lena de Vries, so correcting Indelo's industry on one row means you should correct it on the others too. A quick filter on the Company column (Company is Indelo) finds them all before you edit. A cleaner design would store each company once in its own table and link the customers to it, so a company's details live in a single place — more powerful, but more complex, which is why this course keeps everything in one table." />}}
...
Switch back to your window 🌐 to see Thomas's reply and the resolved marker. You have just run a complete loop — ask, notify, act, confirm, resolve — without ever leaving the data.
...
Test the rule that you can only mention collaborators: start a comment, type `@`, and look at who appears. Thomas is there because you shared the base with him in Step 2 — but a teammate you have *not* shared this base with will not appear, however much they belong to your team. It is the quickest way to see why sharing had to come first.
