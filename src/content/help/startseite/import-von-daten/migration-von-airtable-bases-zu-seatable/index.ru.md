...
Вы хотите перенести свои базы Airtable на SeaTable и боитесь, что это будет сложно? Не волнуйтесь! Даже базы со сложной структурой данных и многими тысячами записей могут быть быстро перенесены в SeaTable. Для этого у вас на выбор есть **два варианта**: простой способ полностью без кода и более гибкий скрипт, для которого пригодятся базовые знания Python.

## Импорт баз Airtable в SeaTable

Для первого способа импорта баз Airtable в SeaTable вам не потребуются никакие знания программирования.

1. Откройте **главную страницу** SeaTable.
2. Нажмите **Добавить базу или папку** в области или группе, где вы хотите разместить новую базу.
![Импорт из Airtable](images/import-from-airtable.png)
3. Выберите **Импорт из Airtable**.
4. Введите [Airtable Personal Access Token (PAT)](https://support.airtable.com/docs/creating-personal-access-tokens), а также [Airtable Base ID](https://support.airtable.com/docs/finding-airtable-ids). Используйте ссылки на документацию Airtable, чтобы узнать, где получить PAT и Base ID. PAT должен обладать правами **data.records:read** и **schema.bases:read**.
![Диалоговое окно импорта Airtable](images/airtable-import-window.png)
5. Задайте **имя базы**, которое новая база получит в SeaTable.
6. Подтвердите кнопкой **Импорт**.

**Посмотрите наше видео с пояснениями и субтитрами на русскими субтитрами.**

{{< youtube 32Vq2R0fzQs >}}

В отличие от скрипта, описанного в следующем разделе, при стандартном способе импорта у вас **нет возможностей для настройки**. Все таблицы и строки (включая вложения) импортируются из Airtable. Поэтому, в зависимости от размера базы Airtable, процесс импорта может занять некоторое время.

Все столбцы Airtable, кроме столбцов типов Button, Count, Lookup и Rollup, также импортируются. Ниже вы найдёте обзор того, как типы столбцов Airtable переносятся в SeaTable. Таблица «Columns to be migrated manually» в новой базе содержит список столбцов, которые не были импортированы из Airtable.

## Как перенести базу из Airtable в SeaTable с помощью скрипта

Для более гибкого переноса данных Airtable мы разработали скрипт миграции. Он переносит выбранные данные из базы Airtable – таблицы, столбцы, записи – в базу SeaTable. После переноса вы можете продолжать работу в SeaTable с данными, которые вы в последний раз использовали в Airtable.

Чтобы подготовить скрипт к работе, требуется лишь несколько **ручных записей**. После выполнения скрипта потребуется ещё немного доработки. В этой статье мы объясним, как это сделать.

{{< warning  type="warning" headline="Важное замечание"  text="Скрипт миграции является функциональным, но все еще бета-версией. Мы постоянно адаптируем и оптимизируем его. Если у вас возникли проблемы, пожалуйста, оставьте сообщение на [форуме SeaTable](https://forum.seatable.com/)." />}}
...
Не волнуйтесь, если вы пока не до конца понимаете код, который вы только что скопировали. Мы объясним его. **Комментарии в коде** должны немного помочь в интерпретации. Комментарии - это все строки с ведущим хэш-знаком ('#'). Эти строки игнорируются при выполнении скрипта, то есть вы можете изменять, удалять или добавлять комментарии, не влияя на функциональность скрипта.
...
Если в базе Airtable нет столбцов ссылок, то квадратную скобку можно оставить пустой:  
...
Теперь выполните сценарий, нажав на кнопку **Выполнить скрипт**. Во время выполнения сценария в редакторе справа видны шаги, которые были выполнены. В фоновом режиме также видно, как создаются таблицы и столбцы.
...
Снова выполните сценарий, нажав на кнопку **Выполнить скрипт**. Вновь можно наблюдать выполнение сценария в правой части и видеть действия в фоновом режиме.
