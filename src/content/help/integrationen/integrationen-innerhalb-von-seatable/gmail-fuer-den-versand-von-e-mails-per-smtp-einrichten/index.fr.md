...
Pour que SeaTable puisse accéder à un compte Gmail via OAuth 2.0, vous devez créer un identifiant et une clé client dans la Cloud Console de Google et les enregistrer dans SeaTable.
...
5. Effectuez les **configurations nécessaires** sur Google Auth Platform : Saisissez un nom d'application, une adresse e-mail d'assistance et une adresse e-mail de contact (par exemple votre adresse Gmail). Sélectionnez "Externe" comme groupe cible. Acceptez les conditions d'utilisation et terminez la configuration.
6. Cliquez sur **Audience** dans la navigation latérale de Google Auth Platform. Ajoutez votre adresse e-mail Google en tant qu'utilisateur test.
7. Dans la navigation latérale de Google Auth Platform, cliquez sur **Clients** et créez un client OAuth 2.0. Sélectionnez "Application web" comme type d'application pour le client OAuth et saisissez un nom (par ex. 'SeaTable'). Sautez les "Sources JavaScript autorisées" et insérez l'URI suivant comme "URI de redirection autorisée" :
...
1. Le client nouvellement créé est maintenant affiché dans la liste des clients OAuth 2.0. Les paramètres du client peuvent être affichés en cliquant sur son nom. L'ID et le code secret du client affichés sont essentiels pour la configuration suivante dans SeaTable.
...
5. Saisissez les informations suivantes :
    ...
    - **Code secret du client**: le code secret du client OAuth créé

6. Cliquez sur **Continuer** lorsqu'il vous est indiqué que cette application n'a pas été vérifiée et autorisez l'accès au compte Google.
